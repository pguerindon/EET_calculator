...
|:--------|:-----|:------------|
| 1.3 | 2026 | Clarified Optional Secondary Requests to allow a variable number of competitors from the secondary timing system, with competitors matched by bib. |
...
This document defines Version 1.3 of the
Electronic Equivalent Time Exchange Protocol (EEP).

This document is the official specification of Version 1.3 of the Electronic Equivalent Time Exchange Protocol (EEP).

Version 1.3 is backward compatible with Version 1.2 for all operations defined by Version 1.2. Optional Secondary Requests that contain exactly 11 competitors remain valid.
...
The Manual Time (MT) may then be provided either through the EET Calculator user interface or by an Optional Secondary Request. An Optional Secondary Request may contain the complete competitor list supplied by the secondary timing system.
...
| Optional Secondary Request | Provide Manual Times from a secondary timing system. |
...
| `competitors`      | array  | Yes      | List of competitors. Initial Requests SHALL contain exactly 11 competitors. Optional Secondary Requests MAY contain a variable number of competitors. |
| `mode`             | string | No       | Processing mode. Recognized values are `""` (or omitted) and `TEST`. |
...
In an Optional Secondary Request, each competitor is identified by its `bib`. The EET Calculator matches the supplied competitor with the corresponding competitor in the Calculation Document using this `bib`. The order of competitors in the array has no semantic meaning.

A competitor present in an Optional Secondary Request but absent from the Calculation Document is ignored. The competitor identified by `eet_bib` is matched in the same way as all other competitors.
...
| **VR-04** | In an Initial Request, the `competitors` array SHALL contain exactly eleven competitors. In an Optional Secondary Request, the `competitors` array MAY contain a variable number of competitors. |
...
For backward compatibility with Version 1.1, an EEP Version 1.3 implementation MAY recognize a request with `mode` set to `DELETE`.
...
The `DELETE` mode is no longer a supported EEP operation in Version 1.3.

# Protocol Examples

The following examples illustrate the HTTP messages exchanged between a timing system and an EET Calculator.

The Optional Secondary Request uses the `calculation_id` assigned by the Initial Request and provides non-empty `et_tod` values for the competitors supplied by the secondary timing system.

The competitor list in an Optional Secondary Request is not required to contain exactly the same competitors as the Initial Request. Competitors are matched by `bib`, and the order of the list has no semantic meaning. The example below uses the same 11 competitors as the Initial Request for simplicity; a secondary timing system MAY provide a larger or otherwise different list.

For clarity, the examples include optional members. Implementations MAY omit any optional member.

Optional competitor members are shown only for the competitor whose electronic timing impulse is missing in the Initial Request.
...
> **Implementation Note**
>
> During processing of an Optional Secondary Request, implementations SHOULD update the Manual Time corresponding to each supplied competitor whose `bib` exists in the Calculation Document. Competitors whose `bib` is not present in the Calculation Document SHOULD be ignored. The order of the supplied competitor list SHOULD NOT affect processing.
