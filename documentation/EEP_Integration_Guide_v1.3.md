...
|:--------|:-----|:------------|
| 1.3 | 2026 | Clarified Optional Secondary Requests to allow the complete competitor list from the secondary timing system, with competitors matched by bib. This version remains applicable to EEP Specification Version 1.2. |
...
Enriches an existing Calculation Document by providing Manual Times (MT) from a secondary timing system.
...
The Optional Secondary Request may contain the complete competitor list supplied by the secondary timing system. It is therefore not restricted to the 11 competitors contained in the Initial Request.

Each competitor is identified by its `bib`. The EET Calculator matches competitors from the Secondary Request with competitors already present in the Calculation Document using this `bib`.

- If a `bib` exists in the Calculation Document, its `et_tod` from the Secondary Request is imported as the corresponding Manual Time.
- If a `bib` is present only in the Secondary Request, it is ignored.
- The order of competitors in the Secondary Request has no significance for the matching process.
- The competitor identified by `eet_bib` is matched in the same way as all other competitors.
- Every competitor supplied in the Secondary Request must contain a non-empty `et_tod`.
...
- For Optional Secondary Requests, send the complete competitor list from the secondary timing system when available.
- Match competitors by `bib`; do not rely on their position in the competitor list.
